    ...
    for O in OVERLAPS_0_1:
        if not len(O.overlaps.keys()) > 2:
            for interval_original in O.overlaps:
                print interval_original.INFO[1],interval_original.start,interval_original.stop
                print d1[interval_original.INFO[1] + ':' + str(interval_original.start) + '-' + str(interval_original.stop)]
