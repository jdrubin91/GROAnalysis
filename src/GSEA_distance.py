...
def create_intersect_file(interval_file,path_to_PSSMs,filedir):
    ...
    for TF in os.listdir(path_to_PSSMs):
        header.append(TF)
        if i == 0:
            os.system("bedtools closest -d -a " + interval_file + " -b " + path_to_PSSMs + TF + " > " + filedir + "all_preliminary_bidir.merge.sort.count.intersect.bed")
        else:
            os.system("bedtools closest -d -a " + interval_file + " -b " + path_to_PSSMs + TF + " > " + filedir + "temp.bed")
...
def gsea_analysis(sorted_intersect_file,figuredir):
    ...
    with open(sorted_intersect_file) as F:
        TFnames = F.readline().strip('\n').split('\t')[4:]
        TFvals = [[] for i in range(len(TFnames))]
        TFsums = [0 for i in range(len(TFnames))]
        for line in F:
            line = line.strip('\n').split('\t')
    ...
    for i in range(len(TFnames)):
        ...
        TF = TFnames[i]
        vals = TFvals[i]
        negs = sum([1 for val in TFvals[i] if val > 1500])
        TFsum = TFsums[i]
        ES = 0
        for tick in vals:
            if tick > H:
