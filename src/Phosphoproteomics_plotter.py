...
def run(file1,protein_sequences,TF1,TF2,figuredir):
            ...
            for TF in TF1:
                if TF == key.split(';')[0]:
                    if float(line[1]) > 1.1 or float(line[1]) < 0.9:
                        if TF in d:
                            d[TF].append((float(line[1]),list(set(line[18].split(';'))),sequences[TF]))
                        else:
                            d[TF] = [(float(line[1]),list(set(line[18].split(';'))),sequences[TF])]

            for TF in TF2:
                if TF == key.split(';')[0]:
                    if float(line[1]) > 1.1 or float(line[1]) < 0.9:
                        if TF in d2:
                            d2[TF].append((float(line[1]),list(set(line[18].split(';'))),sequences[TF]))
                        else:
                            d2[TF] = [((float(line[1])),list(set(line[18].split(';'))),sequences[TF])]
    ...
    #                     colors.append(val)
    #                     # if TF == 'BPTF':
    #                     #     if seq[int(pos)-1] + str(int(pos)) == 'S1769':
    #                     #         ax.text(pos/length-0.03,i*gap+1.9,seq[int(pos)-1] + str(int(pos)),fontsize=fontsize)
    #                     #     elif seq[int(pos)-1] + str(int(pos)) == 'S2465':
    #                     #         ax.text(pos/length-0.03,i*gap+1.9,seq[int(pos)-1] + str(int(pos)),fontsize=fontsize)
    #                     #     elif seq[int(pos)-1] + str(int(pos)) == 'S1300':
    #                     #         ax.text(pos/length-0.03,i*gap+1.9,seq[int(pos)-1] + str(int(pos)),fontsize=fontsize)
    #                     #     elif seq[int(pos)-1] + str(int(pos)) == 'S1310':
    #                     #         ax.text(pos/length-0.03,i*gap-1.4,seq[int(pos)-1] + str(int(pos)),fontsize=fontsize)
    #                     #     else:
    #                     #         ax.text(pos/length-0.03,i*gap+1,seq[int(pos)-1] + str(int(pos)),fontsize=fontsize)
    #                     # else:
    #                     #     ax.text(pos/length-0.03,i*gap+1,seq[int(pos)-1] + str(int(pos)),fontsize=fontsize)
    #                     if TF == 'SMAD3':
    #                         if seq[int(pos)-1] + str(int(pos)) == 'S3':
    #                             ax.text(pos/length+0.02,i*gap+1,seq[int(pos)-1] + str(int(pos)),fontsize=fontsize)
    #                         else:
    #                             ax.text(pos/length-0.03,i*gap+1,seq[int(pos)-1] + str(int(pos)),fontsize=fontsize)
    #                     else:
    #                         ax.text(pos/length-0.03,i*gap+1,seq[int(pos)-1] + str(int(pos)),fontsize=fontsize)
    ...
    #                         ax.text(pos/length-0.03,i*gap+1,seq[int(pos)-1] + str(int(pos)),fontsize=fontsize)
    #         i = i-1
    # plt.scatter(x,y,zorder=1,s=200,edgecolor="",c=colors, cmap=rvb,vmin=0.7,vmax=1.3)
    # ax.set_title('Sites of Differential Phosphorylation')
    ...
    # plt.colorbar()
    # F.savefig(figuredir + '/Phosphoproteomics_JUN_SRF_Interactors.png', dpi=1200)
    # # plt.show()
...
if __name__ == "__main__":
    file1 = '/Users/jonathanrubin/Google Drive/Colorado University/Jonathan/HCT116_Serinduction_phospho_rep1.txt'
